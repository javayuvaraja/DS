 ...
 * Logger Rate Limiter Design a logger system that receive stream of messages
 * along with its timestamps, each message should be printed if and only if it
...
public class RateLimiterLogger {
	...
	Map<String, Integer> timestampMap = new ConcurrentHashMap<>(); 
	int limiter = 10;
	public boolean shouldPrintMessage(int timestamp, String message) {
		boolean canPrint = false;
		if (timestampMap.containsKey(message)) {
			if (!(timestampMap.get(message)+limiter > timestamp)) {
				canPrint = true;
