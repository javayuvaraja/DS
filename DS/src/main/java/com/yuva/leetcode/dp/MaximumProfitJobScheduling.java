...
public class MaximumProfitJobScheduling {
	...
	/**
	 *  1. Sort the interval based on the end time
	 *  1. Key as End time and value as Profit
	 *  2. Binary search in the dp to find the largest profit we can make before start time s.
	...
	public int jobScheduling(int[] startTime, int[] endTime, int[] profit) {
        ...
        dp.put(0, 0);
        for (int[] job : jobs) {
            int cur = dp.floorEntry(job[start]).getValue() + job[prof];
