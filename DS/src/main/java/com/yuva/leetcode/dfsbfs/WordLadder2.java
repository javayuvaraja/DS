...
public class WordLadder2 {
	 ...
	 * calculating distance from beginWord to each node in the graph (should store
	 * minimum distance)
	 * 
	 * Step 2: use DFS to traverse and record path from beginWord to endWord with
	 * shortest path. We can use distance map to control every next word.
	 */
	...
	public void bfs(String beginWord, String endWord, Set<String> dict, Map<String, Set<String>> graph,
		...
		while (!queue.isEmpty()) {
			...
			for (int i = 0; i < size; i++) {
				...
				/*
				 * try all possible substitution (26 characters) in every position of current
				 * word, if newWord exists in dictionary, we add it to the adjacency list of
				 * curWord
				 */
				...
				// traverse all neighbors of current word, update distance map and queue for
				// next ladder (level)
				// WARNING: DO NOT USE visited set, since it is hard to deal with end word if
				// endWord is visited
